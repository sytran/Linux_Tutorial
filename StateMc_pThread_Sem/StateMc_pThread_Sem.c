...
pthread_t thr_conditivity, thr_pH, thr_miliVolt;
sem_t sem_conditivity, sem_pH, sem_miliVolt;
sem_t sem_update_conditivity, sem_update_pH, sem_update_miliVolt;
...
int main() {
  ...
  //
  sem_init(&sem_conditivity, NULL, 0);
  sem_init(&sem_pH, NULL, 0);
  sem_init(&sem_miliVolt, NULL, 0);
  
  sem_init(&sem_conditivity, NULL, 0);
  sem_init(&sem_pH, NULL, 0);
  sem_init(&sem_miliVolt, NULL, 0);

  sem_init(&sem_update_conditivity, NULL, 0);
  sem_init(&sem_update_pH, NULL, 0);
  sem_init(&sem_update_miliVolt, NULL, 0);

  // create all threads running in the system
  pthread_create(&id_timer, NULL, timer_event, NULL);
  pthread_create(&id_read, NULL, asciRead, NULL);
  pthread_create(&id_machine, NULL, state_machine, NULL);
